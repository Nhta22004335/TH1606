...
// 1. LẤY ID CỦA MÔI GIỚI ĐANG ĐĂNG NHẬP
$id_moigioi = $_SESSION['id_nguoi_dung'] ?? null; 

// Kiểm tra nếu người dùng chưa đăng nhập, nên chuyển hướng hoặc báo lỗi
if (!$id_moigioi) {
    // Thay thế bằng logic chuyển hướng thực tế của bạn
    // header("Location: ../auth/dangnhap.php"); 
    // exit;
    echo "<p class='p-4 text-red-500 font-bold'>Vui lòng đăng nhập để xem sản phẩm cá nhân.</p>";
    $id_moigioi = 'INVALID_ID'; // Gán ID không hợp lệ để ngăn chặn truy vấn
}
...
// =============================================================
// TRUY VẤN 1: DÙNG CHO DANH SÁCH MẶC ĐỊNH HOẶC KHI TÌM KIẾM RỖNG
// =============================================================
$sql_default = "
    SELECT 
        b.id,
        ...
        -- Lấy URL ảnh đại diện (ảnh đầu tiên)
        COALESCE(ha.url, 'chuacapnhat.jpg') AS anh_dai_dien 
        
    FROM 
        public.bat_dong_san b
    LEFT JOIN 
        danh_gia_bds d ON d.id_bds = b.id
    
    -- LATERAL JOIN để lấy duy nhất một hình ảnh có ngày tạo sớm nhất
    LEFT JOIN LATERAL (
        SELECT url
        FROM hinh_anh_bds 
        WHERE id_bds = b.id
    ...
    WHERE 
        b.id_nguoi_dung = :id_moigioi  -- LỌC THEO ID MÔI GIỚI
    ORDER BY 
        b.ngay_dang DESC
";
...
// =============================================================
// TRUY VẤN 2: DÙNG CHO TÌM KIẾM FULL-TEXT (PostgreSQL)
// =============================================================
if (!empty($search)) {
    $sql_search = "
        SELECT 
            ...
            d.diem as rating,
            ts_rank_cd(
                to_tsvector('simple', b.tieu_de || ' ' || b.mo_ta), 
                plainto_tsquery('simple', :search_query)
            ) AS rank
        FROM public.bat_dong_san b
        LEFT JOIN danh_gia_bds d ON d.id_bds = b.id
        WHERE 
            b.id_nguoi_dung = :id_moigioi AND -- LỌC THEO ID MÔI GIỚI
            to_tsvector('simple', b.tieu_de || ' ' || b.mo_ta) @@ plainto_tsquery('simple', :search_query)
    ...
    // Nếu không tìm kiếm, sử dụng kết quả từ truy vấn mặc định
    $mangtksanpham = $products;
}

// Bổ sung: Điều chỉnh truy vấn tìm kiếm để tránh nối chuỗi sai trong to_tsvector
// Thay thế to_tsvector('simple', b.tieu_de || :search || b.mo_ta) 
// Bằng: to_tsvector('simple', b.tieu_de || ' ' || b.mo_ta) và truyền tham số tìm kiếm riêng
?>
<!DOCTYPE html>
            ...
            <?php if (empty($filters) && empty($mangtksanpham)): ?>
                <?php foreach($products as $p): ?>
                    <div class="bg-white shadow rounded-xl overflow-hidden hover:shadow-lg transition flex flex-col">
                        <img src="../../../storage/bds/<?= htmlspecialchars($m['anh_dai_dien'] ?? 'chuacapnhat.jpg') ?>" 
                            class="w-full h-40 object-cover cursor-pointer">
                            <?php echo '<!-- ' . $m['anh_dai_dien'] . ' -->'; ?>
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <p class="mt-1 text-xs text-gray-600">
                                <?php
                                    ...
                                    echo $hinh_thuc[$p['hinh_thuc']] ?? 'Chưa cập nhật';
                                ?> - <?= $p['khu_vuc'] ?>
                            </p>
                            <?php
                                $trangThai = [
                            ...
                            <p class="mt-1 text-xs text-gray-600">Loại: <?= $p['loai'] ?></p>
                            ...
                            <div class="p-4 flex-1 flex flex-col">
                                ...
                                <p class="mt-1 text-xs text-gray-600">
                                    <?php
                                        ...
                                        echo $hinh_thuc[$p['loai']] ?? 'Chưa cập nhật';
                                    ?> - <?= $p['khu_vuc'] ?>
                                </p>
                                <?php
                                    $trangThai = [
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <p class="mt-1 text-xs text-gray-600">
                                <?php
                                    ...
                                    echo $hinh_thuc[$m['loai']] ?? 'Chưa cập nhật';
                                ?> - <?= $m['khu_vuc'] ?>
                            </p>
                            <?php
                                $trangThai = [
                                    'daduyet' => 'Chờ duyệt',
                                    'chuaduyet'  => 'Đang bán',
                                    'chuagiaodich'  => 'Chưa giao dịch',
                                    'daban'    => 'Đã bán',
                                    'dathue'   => 'Đã thuê'
                                ];
                            ?>
                            <p class="mt-1 text-xs text-gray-600 font-semibold">Trạng thái: <?= $trangThai[$m['trang_thai']]?></p>
                            <p class="mt-1 text-xs text-gray-600 font-semibold">Loại: <?= $m['loai']?></p>
