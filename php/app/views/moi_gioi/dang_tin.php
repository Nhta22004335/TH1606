    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['capnhattintuc'])) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $duongDan)) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tin_tuc (id_khach_hang, tieu_de, mo_ta, chuyen_muc, trang_thai, anh_tin) 
                VALUES (?, ?, ?, ?, ?, ?)");
                $result = $stmt->execute([$_SESSION['id_nguoi_dung'], $tieu_de, $mo_ta, $chuyen_muc, $trang_thai, $tenFileMoi]);
            } else {
...
<body class="bg-gray-100">
    ...
    <div>
        ...
        <input type="text" name="tieu_de"  class="w-full border px-3 py-2 rounded">
    </div>

    <div>
        <label class="block font-semibold">Mô tả:</label>
        <textarea name="mo_ta" class="w-full border px-3 py-2 rounded"></textarea>
    </div>

    <div>
        <label class="block font-semibold">Chuyên mục:</label>
        <input type="text" name="chuyen_muc" class="w-full border px-3 py-2 rounded">
    </div>
