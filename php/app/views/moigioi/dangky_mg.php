...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vai_tro = $_POST['vai_tro'] ?? 'khachhang'; // mặc định khách hàng
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Chèn vào bảng chung nguoi_dung
        $stmt = $pdo->prepare("INSERT INTO nguoi_dung (ten, email, matkhau, phone, dia_chi, vai_tro) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$ten, $email, $matkhau, $phone, $dia_chi, $vai_tro]);
...
<head>
    <meta charset="UTF-8">
    <title>Đăng ký Nhà Đẹp 24h</title>
...
<div class="relative z-10 bg-white bg-opacity-90 backdrop-blur-md p-6 w-full max-w-sm rounded-3xl shadow-2xl">
    <h2 class="text-3xl font-bold text-center text-blue-600 mb-6">Đăng ký Nhà Đẹp 24h</h2>
    ...
    <form action="" method="post" class="space-y-3" autocomplete="off">
        ...
        <div>
            <label class="block font-semibold mb-1" for="vai_tro">Vai trò</label>
