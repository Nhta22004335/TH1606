...
$sql_base = "
    ...
    WHERE b.trang_thai = 'daduyet' -- chỉ hiện tin đã duyệt
      AND (
        LOWER(b.loai) ILIKE ANY (ARRAY[ 'nhà', 'nha', 'căn hộ', 'canho' ]::text[]) -- placeholder, we'll construct below
      )
";
...
// Start full select
$sql_count = "SELECT COUNT(*) " . " FROM bat_dong_san b JOIN nguoi_dung u ON u.id = b.id_nguoi_dung WHERE b.trang_thai = 'daduyet' AND ($loai_cond) ";
        ...
        <?php else: ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($rows as $p): 
                    $img = !empty($p['anh_dai_dien']) ? "/storage/bds/" . e($p['anh_dai_dien']) : "/public/assets/no-image.png";
                ?>
