...
// ----- schema helpers -----
function tableCols(PDO $pdo, $table){
  $stmt=$pdo->prepare("SELECT lower(column_name) AS c FROM information_schema.columns WHERE table_schema='public' AND table_name=:t");
  $stmt->execute([':t'=>strtolower($table)]);
  return array_column($stmt->fetchAll(PDO::FETCH_ASSOC),'c', 'c'); // set for fast exists
}
function pick($cols, $cands){
  foreach($cands as $c){ if(isset($cols[strtolower($c)])) return strtolower($c); }
  return null;
}

$bCols = tableCols($pdo,'bat_dong_san');
$uCols = tableCols($pdo,'nguoi_dung');

// map columns
$ownerCol = pick($bCols, ['id_chu_so_huu','id_nguoi_dung','id_owner','id_chu_nha']);
$statusCol= pick($bCols, ['trang_thai','status','tinh_trang']);
$titleCol = pick($bCols, ['tieu_de','tieude','ten','ten_bds','tieu_de_tin','title','mo_ta','mota','noi_dung']);
$areaCol  = pick($bCols, ['khu_vuc','khuvuc','vi_tri','thanh_pho','tinh_thanh','quan_huyen','khu']);
$addrCol  = pick($bCols, ['dia_chi','diachi','dia_chi_cu_the','address']);
$priceCol = pick($bCols, ['gia','gia_ban','gia_tien','gia_bds','price']);
$sizeCol  = pick($bCols, ['dien_tich','dientich','dien_tich_m2','dien_tich_dat','dien_tich_su_dung']);
$dateCol  = pick($bCols, ['ngay_dang','created_at','ngay_tao','ngaydang']);

$uNameCol = pick($uCols, ['ten_dang_nhap','ho_ten','username','email','full_name']);
$uPhoneCol= pick($uCols, ['so_dt','sdt','dien_thoai','phone']);
$uAvtCol  = pick($uCols, ['avt','avatar','anh_dai_dien']);
...
// keywords cho Nhà & Căn hộ
$kwList = ['%nhà %','%nha %','%căn hộ%','%can ho%','%chung cư%','%chung cu%','%nhà phố%','%nha pho%','%apartment%','%condo%'];

// chọn cột để match keyword
$searchCols = array_values(array_filter([$titleCol,$areaCol,$addrCol]));
if (!$searchCols) { // không có cột text => bỏ điều kiện loại
  $searchCols = [];
}
...
// build OR keyword across available columns
if ($searchCols) {
  $orParts = [];
  $i=0;
  foreach ($searchCols as $c) {
    foreach ($kwList as $ki => $kw) {
      $p = ":kw{$i}_{$ki}";
      $orParts[] = "b.\"$c\" ILIKE $p";
      $params[$p] = $kw;
    }
...
if ($search !== '' && $searchCols){
  $free = [];
  foreach ($searchCols as $idx => $c){
    $free[] = "b.\"$c\" ILIKE :search";
  }
  $where[] = '(' . implode(' OR ', $free) . ')';
  $params[':search'] = "%$search%";
}
$whereSql = $where ? (' WHERE ' . implode(' AND ', $where)) : '';

$sql_count = "SELECT COUNT(*) FROM bat_dong_san b" . $whereSql;
...
if ($ownerCol) {
  ...
  $select[] = "''::text AS ten_dang_nhap";
  $select[] = "''::text AS so_dt";
  $select[] = "''::text AS avt";
}
...
<!DOCTYPE html>
<html lang="vi">
<head><meta charset="utf-8"><title>Nhà ở & Căn hộ</title><script src="https://cdn.tailwindcss.com"></script></head>
...
<div class="max-w-7xl mx-auto px-4 py-8">
  ...
  <?php if (!$rows): ?>
    <p class="text-gray-500">Chưa có sản phẩm phù hợp.</p>
  <?php else: ?>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <?php foreach ($rows as $p):
        $img = $imgPrefix . e($p['anh_dai_dien']); ?>
