                    ...
                    <?php else: ?>
                        ...
                        ?>
                            <tr class="hover:bg-blue-50/50 transition duration-150">
                                ...
                                <td class="p-4 text-sm text-gray-600 text-center flex justify-center gap-2">
                                <td class="p-4 text-sm text-gray-600 text-center flex justify-center gap-2">
                                <!-- Nút Xem -->
                                <button data-modal-toggle="docModal<?= $bm['id'] ?>" class="bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium px-4 py-2 rounded-lg flex items-center justify-center gap-1.5 shadow-md transition transform hover:scale-105">
...
document.addEventListener('click', function(e) {
    ...
    fetch("../../models/xoa_bm.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: `id=${encodeURIComponent(id)}`
    })
    .then(res => res.ok ? res.json() : Promise.reject(`Lỗi HTTP: ${res.status}`))
    .then(data => {
        alert(data.message);
        if (data.status === "success") {
            // Xóa row trong bảng ngay lập tức
